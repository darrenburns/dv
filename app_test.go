...
func TestDv_RightPaneUsesPaddedEmptyStateWhenNoDiffs(tt *testing.T) {
	...
	expectedForeground := viewerEmptySpaceBackground(theme)
	emptyFill, ok := column.Children[2].(viewerEmptySpaceHatch)
	require.True(tt, ok, "expected a hatch filler widget below short right-pane content")
	require.NotNil(tt, emptyFill.Style.BackgroundColor)
	require.Equal(tt, theme.Background, emptyFill.Style.BackgroundColor.ColorAt(20, 20, 0, 0))
	require.NotNil(tt, emptyFill.Foreground)
	require.Equal(tt, expectedForeground.ColorAt(20, 20, 0, 0), emptyFill.Foreground.ColorAt(20, 20, 0, 0))
	require.Equal(tt, expectedForeground.ColorAt(20, 20, 19, 19), emptyFill.Foreground.ColorAt(20, 20, 19, 19))
